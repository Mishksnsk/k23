import requests
import time
requests.get("https://CommunityApi-1.probot8.repl.co").text
time.sleep(5)
api = requests.get("https://CommunityApi-1.probot8.repl.co").text
parameters = {
    "community-link": api#,#"blog-link":blog-api
}
import uuid
import binascii
import os
...
class Client:
    ...
    def generate_signature_message(self, data):
        signature_message = b64encode(bytes.fromhex("42") + hmac.new(bytes.fromhex("F8E7A61AC3F725941E3AC7CAE2D688BE97F30B93"),data.encode("utf-8"), sha1).digest()).decode("utf-8")
        self.headers["NDC-MSG-SIG"]=signature_message
        return signature_message

    def generate_device_Id(self):
        identifier = os.urandom(20)
        mac = hmac.new(bytes.fromhex("02B258C63559D8804321C5D5065AF320358D366F"), bytes.fromhex("42") + identifier, sha1)
